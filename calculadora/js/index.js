...
function pedirOperador() {
    ...
    do {
        operador = prompt("Introduce el operador de la operación que quieres realizar. (+, -, * ó /)");
        if(operador == null){
            break;
        }
        if (!comprobarOperador(operador)) {
            alert("El operador introducido es incorrecto. Debe ser (+, -, * ó /)")
        }
...
function mostrarResultado(operador, operando1, operando2) {
    ...
    switch (operador) {
        case "+":
            alert(`El resultado ${calculadora.suma(operando1, operando2)}`);
            break;
        case "-":
            alert(`El resultado ${calculadora.resta(operando1, operando2)}`);
            break;
        case "/":
            //Se comprueba que no se pueda dividir entre 0
            if (operando2 === 0) {
                alert("Lo siento no se puede dividir ningún número entre 0");
            }
            else {
                alert(`El resultado ${calculadora.division(operando1, operando2)}`);
            }
            break;
        case "*":
            alert(`El resultado ${calculadora.multiplicacion(operando1, operando2)}`);
    }
}
...
outer:do {
    ...
    do {
        ...
        operandos = prompt("Introduce los operandos separados por un espacio");
        ...
        if (pedirPrimerOperando(operandos) === -1 || pedirSegundoOperando(operandos) === -1) {
            alert("Debes introducir dos números separados por un espacio");
        }
